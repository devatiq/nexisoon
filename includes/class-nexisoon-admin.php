...
class NexiSoon_Admin
{
	...
	{
		$titles = array(
			'coming-soon' => esc_html__('NexiSoon: Coming Soon Active', 'nexisoon'),
			'under-construction' => esc_html__('NexiSoon: Under Construction Active', 'nexisoon'),
			'maintenance' => esc_html__('NexiSoon: Maintenance Active', 'nexisoon'),
		);
	...
	{
		return array(
			'general' => esc_html__('General', 'nexisoon'),
			'template' => esc_html__('Template', 'nexisoon'),
			'branding' => esc_html__('Branding', 'nexisoon'),
			'content' => esc_html__('Content', 'nexisoon'),
			'countdown' => esc_html__('Countdown', 'nexisoon'),
			'seo' => esc_html__('SEO', 'nexisoon'),
			'styling' => esc_html__('Styling', 'nexisoon'),
			'preview' => esc_html__('Preview', 'nexisoon'),
		);
	}
	...
	public function render_settings_page()
	{
		...
		$status_label = !empty($settings['enabled']) ? esc_html__('Active', 'nexisoon') : esc_html__('Disabled', 'nexisoon');
		...
		<div class="wrap nexisoon-wrap">
			<div class="nexisoon-page-header">
				...
				<div>
					...
					<p><?php esc_html_e('Create a temporary frontend page while your real website stays available to administrators.', 'nexisoon'); ?>
					</p>
				</div>
				<span id="nexisoon-status-badge"
					class="nexisoon-status-badge <?php echo esc_attr($status_class); ?>"><?php echo esc_html($status_label); ?></span>
			</div>
			...
			<form id="nexisoon-settings-form" class="nexisoon-settings-form" method="post" action="#">
				<input type="hidden" name="nexisoon_nonce"
					value="<?php echo esc_attr(wp_create_nonce('nexisoon_save_settings')); ?>" />
				...
				<div class="nexisoon-save-bar">
					<div class="nexisoon-save-actions">
						...
						<button type="submit" class="button button-primary button-large"
							id="nexisoon-save-button"><?php esc_html_e('Save Settings', 'nexisoon'); ?></button>
					</div>
				</div>
			</form>
		</div>
		...
		<nav class="nav-tab-wrapper nexisoon-tabs" aria-label="<?php esc_attr_e('NexiSoon settings tabs', 'nexisoon'); ?>">
			<?php foreach ($tabs as $tab => $label): ?>
				<a class="nav-tab <?php echo $current_tab === $tab ? 'nav-tab-active' : ''; ?>"
					href="<?php echo esc_url(add_query_arg(array('page' => 'nexisoon', 'tab' => $tab), admin_url('admin.php'))); ?>"
					data-nexisoon-tab-link="<?php echo esc_attr($tab); ?>">
	...
	private function render_tab_panel($tab, $current_tab, $callback, $args)
	{
		...
		?>
		<section class="nexisoon-tab-panel <?php echo $is_active ? 'is-active' : ''; ?>"
			id="nexisoon-tab-<?php echo esc_attr($tab); ?>" data-nexisoon-tab-panel="<?php echo esc_attr($tab); ?>" <?php echo $is_active ? '' : 'hidden'; ?>>
	...
	private function render_general_tab($args)
	{
		...
		<div class="nexisoon-card nexisoon-card-hero">
			<div>
				...
				<p><?php esc_html_e('Turn NexiSoon on when public visitors should see your temporary page instead of the live website.', 'nexisoon'); ?>
				</p>
			</div>
			<span class="nexisoon-status-badge <?php echo !empty($settings['enabled']) ? 'is-active' : 'is-disabled'; ?>"
				data-nexisoon-status-preview><?php echo !empty($settings['enabled']) ? esc_html__('Active', 'nexisoon') : esc_html__('Disabled', 'nexisoon'); ?></span>
		</div>
				...
				<tbody>
					<tr>
						...
						<td>
							...
							<p class="description">
								<?php esc_html_e('When disabled, NexiSoon does not affect the frontend.', 'nexisoon'); ?></p>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<p class="description">
								<?php esc_html_e('Coming Soon and Under Construction return HTTP 200. Maintenance Mode returns HTTP 503.', 'nexisoon'); ?>
							</p>
						</td>
					</tr>
				</tbody>
		...
		<div class="nexisoon-notice-card">
			...
			<div>
				...
				<p><?php esc_html_e('Logged-in administrators with the manage_options capability continue to see the real website while public visitors see NexiSoon.', 'nexisoon'); ?>
				</p>
			</div>
		</div>
		<?php
	...
	private function render_template_tab($args)
	{
		...
		<div class="nexisoon-card nexisoon-template-picker-card">
			...
			<p><?php esc_html_e('Choose one of the three bundled layouts. Click a preview card to make it the active NexiSoon template.', 'nexisoon'); ?>
			</p>

			<div class="nexisoon-template-grid" role="radiogroup"
				aria-label="<?php esc_attr_e('Template selector', 'nexisoon'); ?>">
				<?php foreach ($templates as $value => $label): ?>
					<?php $is_selected = $settings['template'] === $value; ?>
					<label class="nexisoon-template-card <?php echo $is_selected ? 'is-selected' : ''; ?>"
						for="nexisoon_template_<?php echo esc_attr($value); ?>"
						data-template-card="<?php echo esc_attr($value); ?>" role="radio"
						aria-checked="<?php echo $is_selected ? 'true' : 'false'; ?>" tabindex="0">
						<input class="nexisoon-template-input" type="radio" id="nexisoon_template_<?php echo esc_attr($value); ?>"
							name="nexisoon_settings[template]" value="<?php echo esc_attr($value); ?>" <?php checked($is_selected); ?> />
						<span class="nexisoon-template-selected-indicator" aria-hidden="true">
							<span class="dashicons dashicons-yes"></span>
						</span>
						<span class="nexisoon-template-preview nexisoon-template-preview-<?php echo esc_attr($value); ?>"
							aria-hidden="true">
						...
						<span class="nexisoon-template-card-body">
							...
							<span
								class="nexisoon-template-choose"><?php echo $is_selected ? esc_html__('Selected', 'nexisoon') : esc_html__('Select template', 'nexisoon'); ?></span>
						</span>
					</label>
				<?php endforeach; ?>
			</div>
	...
	private function render_branding_tab($args)
	{
		...
		<div class="nexisoon-card">
			...
			<p><?php esc_html_e('Use media from your WordPress library. Uploaded files remain in the media library if NexiSoon is deleted.', 'nexisoon'); ?>
			</p>
			<table class="form-table" role="presentation">
				<tbody>
					<?php
	...
	private function render_content_tab($args)
	{
		...
		<div class="nexisoon-card">
			...
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						<th scope="row"><label for="nexisoon_heading"><?php esc_html_e('Heading text', 'nexisoon'); ?></label>
						</th>
						<td><input type="text" class="regular-text" id="nexisoon_heading" name="nexisoon_settings[heading]"
								value="<?php echo esc_attr($settings['heading']); ?>" /></td>
					</tr>
					<tr>
						<th scope="row"><label
								for="nexisoon_subheading"><?php esc_html_e('Subheading text', 'nexisoon'); ?></label></th>
						<td><textarea class="large-text" rows="4" id="nexisoon_subheading"
								name="nexisoon_settings[subheading]"><?php echo esc_textarea($settings['subheading']); ?></textarea>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<th scope="row"><label
								for="nexisoon_button_text"><?php esc_html_e('Button text', 'nexisoon'); ?></label></th>
						<td><input type="text" class="regular-text" id="nexisoon_button_text"
								name="nexisoon_settings[button_text]"
								value="<?php echo esc_attr($settings['button_text']); ?>" /></td>
					</tr>
					<tr>
						<th scope="row"><label
								for="nexisoon_button_url"><?php esc_html_e('Button URL', 'nexisoon'); ?></label></th>
						<td><input type="url" class="regular-text" id="nexisoon_button_url" name="nexisoon_settings[button_url]"
								value="<?php echo esc_url($settings['button_url']); ?>" placeholder="https://example.com" />
						</td>
					</tr>
					<tr>
						<th scope="row"><label
								for="nexisoon_footer_text"><?php esc_html_e('Footer text', 'nexisoon'); ?></label></th>
						<td>
							<textarea class="large-text" rows="3" id="nexisoon_footer_text"
								name="nexisoon_settings[footer_text]"><?php echo esc_textarea($settings['footer_text']); ?></textarea>
							<p class="description">
								<?php esc_html_e('Optional. Leave blank to hide footer text or add your own credit/message.', 'nexisoon'); ?>
							</p>
						</td>
					</tr>
				</tbody>
		...
		<div class="nexisoon-card">
			...
			<table class="form-table" role="presentation">
				<tbody>
					...
					<tr>
						<th scope="row"><label
								for="nexisoon_countdown_datetime"><?php esc_html_e('Countdown date/time', 'nexisoon'); ?></label>
						</th>
						<td><input type="datetime-local" id="nexisoon_countdown_datetime"
								name="nexisoon_settings[countdown_datetime]"
								value="<?php echo esc_attr($settings['countdown_datetime']); ?>" /></td>
					</tr>
					<tr>
						...
						<td>
							...
							<p class="description">
								<?php esc_html_e('This setting currently updates the admin preview and prepares the option for future template controls.', 'nexisoon'); ?>
							</p>
						</td>
					</tr>
				</tbody>
	...
	private function render_seo_tab($args)
	{
		...
		<div class="nexisoon-card">
			...
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						<th scope="row"><label
								for="nexisoon_seo_title"><?php esc_html_e('SEO meta title', 'nexisoon'); ?></label></th>
						<td><input type="text" class="regular-text" id="nexisoon_seo_title" name="nexisoon_settings[seo_title]"
								value="<?php echo esc_attr($settings['seo_title']); ?>" /></td>
					</tr>
					<tr>
						<th scope="row"><label
								for="nexisoon_seo_description"><?php esc_html_e('SEO meta description', 'nexisoon'); ?></label>
						</th>
						<td><textarea class="large-text" rows="4" id="nexisoon_seo_description"
								name="nexisoon_settings[seo_description]"><?php echo esc_textarea($settings['seo_description']); ?></textarea>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="nexisoon-notice-card">
			...
			<div>
				...
				<p><?php esc_html_e('Coming Soon and Under Construction can be indexed normally. Maintenance Mode automatically adds noindex, nofollow to protect temporary outage pages from indexing.', 'nexisoon'); ?>
				</p>
			</div>
		</div>
		<?php
		...
		<div class="nexisoon-card">
			...
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						<th scope="row"><label
								for="nexisoon_background_color"><?php esc_html_e('Background color', 'nexisoon'); ?></label>
						</th>
						<td><input type="text" class="nexisoon-color-field" id="nexisoon_background_color"
								name="nexisoon_settings[background_color]"
								value="<?php echo esc_attr($settings['background_color']); ?>" data-default-color="#111827" />
						</td>
					</tr>
					<tr>
						<th scope="row"><label
								for="nexisoon_text_color"><?php esc_html_e('Text color', 'nexisoon'); ?></label></th>
						<td><input type="text" class="nexisoon-color-field" id="nexisoon_text_color"
								name="nexisoon_settings[text_color]" value="<?php echo esc_attr($settings['text_color']); ?>"
								data-default-color="#ffffff" /></td>
					</tr>
					<tr>
						<th scope="row"><label
								for="nexisoon_button_color"><?php esc_html_e('Button color', 'nexisoon'); ?></label></th>
						<td><input type="text" class="nexisoon-color-field" id="nexisoon_button_color"
								name="nexisoon_settings[button_color]"
								value="<?php echo esc_attr($settings['button_color']); ?>" data-default-color="#2563eb" />
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="nexisoon-card nexisoon-preview-tab-card">
			<div class="nexisoon-preview-header">
				...
				<a class="button" href="<?php echo esc_url($preview_url); ?>" target="_blank"
					rel="noopener noreferrer"><?php esc_html_e('Preview NexiSoon Page', 'nexisoon'); ?></a>
			</div>
	...
	public function ajax_save_settings()
	{
		if (!check_ajax_referer('nexisoon_save_settings', 'nexisoon_nonce', false)) {
			wp_send_json_error(
				array(
					'message' => esc_html__('Security check failed. Please refresh the page and try again.', 'nexisoon'),
				),
				403
			);
		}
		...
		if (!current_user_can('manage_options')) {
			wp_send_json_error(
				array(
					'message' => esc_html__('You do not have permission to save these settings.', 'nexisoon'),
				),
				403
			);
		}
		...
		wp_send_json_success(
			array(
				'message' => esc_html__('Settings saved.', 'nexisoon'),
				'settings' => $saved,
				'logo_url' => $logo_url,
			...
			<td>
				<div class="nexisoon-media-control">
					<input type="hidden" id="<?php echo esc_attr($id); ?>"
						name="nexisoon_settings[<?php echo esc_attr($name); ?>]"
					...
					<div>
						<div class="nexisoon-media-row">
							<button type="button" class="button nexisoon-media-upload"
								data-target="<?php echo esc_attr($id); ?>"
								data-preview="<?php echo esc_attr($preview_id); ?>"
								data-kind="<?php echo esc_attr($name); ?>">
								<?php echo esc_html($select_label); ?>
							</button>
							<button type="button" class="button nexisoon-media-remove"
								data-target="<?php echo esc_attr($id); ?>"
								data-preview="<?php echo esc_attr($preview_id); ?>">
	...
	{
		$descriptions = array(
			'classic-centered' => esc_html__('Balanced centered layout with a polished card.', 'nexisoon'),
			'minimal-launch' => esc_html__('Clean editorial layout with left-aligned content.', 'nexisoon'),
			'bold-countdown' => esc_html__('High-impact layout built around the countdown.', 'nexisoon'),
		);
	...
	private function render_live_preview($settings, $logo_url, $large = false)
	{
		...
		<div class="<?php echo esc_attr($classes); ?>"
			data-preview-template="<?php echo esc_attr($settings['template']); ?>"
			style="background-color: <?php echo esc_attr($settings['background_color']); ?>; color: <?php echo esc_attr($settings['text_color']); ?>;">
			...
			<div class="nexisoon-preview-countdown <?php echo (isset($settings['countdown_display_style']) && 'inline' === $settings['countdown_display_style']) ? 'is-inline' : ''; ?>"
				data-preview-countdown><?php esc_html_e('00 days 00 hours 00 minutes', 'nexisoon'); ?></div>
			<a class="nexisoon-preview-button<?php echo $settings['button_enabled'] ? '' : ' is-hidden'; ?>" href="#"
				style="background-color: <?php echo esc_attr($settings['button_color']); ?>;" data-preview-button>
